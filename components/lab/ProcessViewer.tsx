        ...
        <div className="mt-1 md:mt-2 pb-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-2 gap-2">
                ...
                <div className="flex gap-1.5">
                    <button onClick={togglePlay} className="flex-1 md:flex-none flex items-center justify-center gap-1.5 px-2 py-1.5 bg-[#7B1113] hover:bg-[#901416] text-white rounded transition-colors border border-[#7B1113] active:scale-95 touch-manipulation shadow-lg shadow-red-900/10 text-xs">
                        {isPlaying ? <Pause size={16} /> : <Play size={16} />}
                        {isPlaying ? ui.pause : ui.play}
                    </button>
                    <button onClick={resetSim} className={`p-1.5 rounded border active:scale-95 touch-manipulation shadow-sm ${theme === 'dark' ? 'bg-slate-800 border-slate-700 text-gray-300 hover:bg-slate-700' : 'bg-white hover:bg-gray-50 text-gray-500 border-gray-200'}`}>
                        <RotateCcw size={16} />
                    </button>
                </div>
            </div>

            <div ref={scrollContainerRef} className="relative border-b pb-8 overflow-x-auto scrollbar-hide snap-x border-gray-200 dark:border-gray-800">
                    ...
                    {stages.map((stage, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center border-2 shadow-md transition-all duration-300 ${isActive ? 'border-amber-500 bg-[#7B1113] text-amber-100 shadow-lg' : (theme === 'dark' ? 'border-slate-700 bg-slate-800 text-gray-500' : 'border-gray-200 bg-white text-gray-400')}`}>
            ...
            <div className={`mt-2 min-h-[100px] border rounded-lg p-3 md:p-4 transition-all flex items-center justify-center shadow-md ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
                {activeStageId ? (
                    (() => {
                        ...
                        return (
                            <div className="animate-fade-in w-full max-w-3xl flex flex-col md:flex-row gap-4 md:gap-6 items-start">
                                ...
                                <div className="w-full">
                                    <h4 className={`text-lg md:text-xl font-serif mb-0 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>{stage?.paliName}</h4>
