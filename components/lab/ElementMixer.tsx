...
const ElementMixer: React.FC<ElementMixerProps> = ({ language, theme }) => {
    ...
    return (
        <div className="mt-3 pb-8 max-w-3xl mx-auto">
            <div className="flex gap-2 mb-3 justify-center">
                <button onClick={() => setActiveTab('visual')} className={`px-3 py-1.5 rounded-full text-xs font-bold transition flex items-center gap-2 ${activeTab === 'visual' ? (theme === 'dark' ? 'bg-gray-100 text-gray-900' : 'bg-gray-900 text-white') : (theme === 'dark' ? 'bg-slate-800 text-gray-400' : 'bg-gray-100 text-gray-500')}`}><Eye size={14} /> {ui.visual}</button>
                <button onClick={() => setActiveTab('analysis')} className={`px-3 py-1.5 rounded-full text-xs font-bold transition flex items-center gap-2 ${activeTab === 'analysis' ? (theme === 'dark' ? 'bg-gray-100 text-gray-900' : 'bg-gray-900 text-white') : (theme === 'dark' ? 'bg-slate-800 text-gray-400' : 'bg-gray-100 text-gray-500')}`}><Microscope size={14} /> {ui.analysis}</button>
            </div>
            ...
            <div className={`border rounded-xl overflow-hidden shadow-lg transition-all duration-500 ${theme === 'dark' ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
                ...
                <div className={`p-4 flex flex-col items-center justify-center relative min-h-[300px] transition-colors duration-700 ${info.bg} overflow-hidden`}>
                    ...
                    <div className="relative w-56 h-56 flex items-center justify-center perspective-1000">
                        {/* AIR */}
                    ...
                    {/* INFO OVERLAY */}
                    <div className="mt-3 text-center relative z-10 max-w-lg animate-fade-in">
                        <h2 className={`text-2xl font-serif font-bold mb-0.5 ${info.color}`}>{info.title}</h2>
                        ...
                        {activeTab === 'analysis' ? (
                            ...
                            <div className={`backdrop-blur-sm rounded-xl p-4 border shadow-sm relative ${theme === 'dark' ? 'bg-slate-900/60 border-slate-700' : 'bg-white/60 border-white/50'}`}>
                                ...
                                <button
                                    onClick={handleTouch}
                                    className={`mt-2 px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-2 mx-auto shadow-sm active:scale-95 transition ${theme === 'dark' ? 'bg-slate-100 text-slate-900 hover:bg-white' : 'bg-gray-900 hover:bg-black text-white'}`}
                                >
                                    <Fingerprint size={14} /> {ui.touch}
                                </button>
                ...
                <div className={`border-t p-4 ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-gray-50 border-gray-200'}`}>
