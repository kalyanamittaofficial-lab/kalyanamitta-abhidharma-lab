  ...
  return (
    <div className={`flex h-screen w-full font-sans relative overflow-hidden transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-900 text-gray-100' : 'bg-white text-gray-900'}`}>
      ...
      >
        ...
        <div className={`p-4 border-t ${theme === 'dark' ? 'border-slate-800 bg-slate-900' : 'border-gray-100 bg-gray-50/50'}`}>
           <div className="flex gap-2 mb-4">
              <button 
                 onClick={toggleTheme} 
                 className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold border transition-colors ${
                    theme === 'dark' 
      ...
      <main className={`flex-1 flex flex-col min-w-0 relative h-full transition-colors ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`}>
        {/* Top Bar */}
        <header className={`h-16 border-b flex items-center justify-between px-4 sticky top-0 z-20 shadow-sm backdrop-blur transition-colors ${
           theme === 'dark' ? 'border-slate-800 bg-slate-900/95' : 'border-gray-200 bg-white/95'
        }`}>
          <div className="flex items-center gap-3">
            <button 
              onClick={toggleSidebar}
              className={`p-2 rounded-lg transition-colors active:scale-95 ${
                 theme === 'dark' ? 'text-gray-400 hover:text-white hover:bg-slate-800' : 'text-gray-500 hover:text-black hover:bg-gray-100'
              }`}
            >
              <Menu size={24} />
            </button>
          ...
          <button 
            onClick={toggleTutor}
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border transition-colors active:scale-95 touch-manipulation ${
              isTutorOpen 
